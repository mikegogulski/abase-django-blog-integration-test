...
def main():
    if not HAS_SCENARIO:
        print("SKIP Class K: langwatch-scenario not installed (cd .abase && uv sync --extra scenario)")
        sys.exit(77)
    if not HAS_API_KEY:
        print("SKIP Class K: API keys missing (set OPENAI_API_KEY or ANTHROPIC_API_KEY)")
        sys.exit(77)
