...
**Purpose:** Analyze what would be required to rename Beads issue IDs from `abase-*` to `abase-*`. No changes are made; this is analysis only.
...
- **Issues:** All existing issues in `.beads/issues.jsonl` have IDs like `abase-126`, `abase-2cf`, `abase-989.1`, etc.
- **References:** `abase-` appears in ~51 lines of `issues.jsonl`, plus docs (TODO.md, BJN-CURSOR-ANALYSIS, etc.), MCP Agent Mail tests, and handover content.
...
- **Effect:** New issues would get `abase-*` IDs. Existing issues would keep `abase-*` unless migrated.
...
- **Option A — In-place edit:** Manually or via script, replace `abase-` with `abase-` in every `id` field and in every `depends_on_id` / `issue_id` reference in `dependencies` arrays. Must preserve referential integrity (e.g. `abase-126.1` depends on `abase-126` → both become `abase-126.1` and `abase-126`).
...
- **Option C — Keep both:** Leave existing issues as `abase-*`; only new issues use `abase-*`. Creates mixed prefixes.
...
- Git history would show the rename; `discovered_from` and other cross-references in commit messages or external tools might still mention `abase-*`.

### 3. Cross-References in Repo

| Location | What to update |
|----------|----------------|
| `TODO.md` | `abase-989`, `abase-126`, etc. |
| `docs/abase/BJN-CURSOR-ANALYSIS.md` | `abase-989.x` references |
| `docs/abase/MOVE-WORKFLOW-TO-ABASE.md` | Notes about `abase-` prefix |
| `docs/abase/MULTI-AGENT-READINESS.md` | Any `abase-` examples |
...
| Commit messages, PR descriptions | Any `abase-xxx` mentions |

### 4. External Systems

- **Agent Mail:** If project keys, reservations, or thread IDs reference `abase-*` bead IDs, those would need to be updated or allowed to coexist.
- **BV (Beads Viewer):** If it caches or displays issue IDs, may need refresh.
- **CI/CD or scripts:** Any automation that parses `br list` or `bd ready --json` and matches `abase-` would need updates.
...
1. **Create a migration script** that:
   ...
   - Builds a map `abase-xxx` → `abase-xxx`
...
4. **Grep and replace** `abase-` → `abase-` in docs, TODO, tests, handover (with care for false positives)
5. **Verify:** `br list`, `br ready`, dependency graph; run `.abase/tests/run_tests.sh`

---

## Why Keep `abase-` (Current Choice)

Per MOVE-WORKFLOW-TO-ABASE.md: *"Keep abase- prefix for continuity after moving workflow → abase"*. Benefits:
